...
let write_attribute_value buf (value : Dwarf_value.t) (form : Dwarf_form.t) str_offsets relocs =
...
let rec write_die buf die die_map str_offsets relocs_ref =
  ...
  List.iter (fun (attr : Proto_die.attribute) ->
    write_attribute_value buf attr.value attr.form str_offsets relocs_ref
  ) (Proto_die.attributes die);
  (* Recursively write children *)
  List.iter (fun child ->
    write_die buf child die_map str_offsets relocs_ref
...
let emit_debug_info t =
  let buf = Buffer.create 2048 in

  (* Build string table and get offset map *)
  let cu = compilation_unit t in
  let cu_with_children = Proto_die.add_children cu (all_dies t) in
  let strings = collect_strings cu_with_children [] in
  ...
  (* Track relocations for label addresses *)
  ...
  write_die die_buf cu_with_children die_map str_offsets relocs_ref;
  ...
  let relocs = List.map (fun r -> { r with offset = r.offset + cu_header_size }) (List.rev !relocs_ref) in

  (Bytes.of_string (Buffer.contents buf), relocs)
...
let emit t =
  ...
  in
  let debug_info_bytes, debug_info_relocs = emit_debug_info t in
  {
