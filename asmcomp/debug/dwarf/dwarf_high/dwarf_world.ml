...
(* Build string table and return (bytes, offset_map, label_to_string_and_offset)
...
let build_string_table strings =
  ...
  (Bytes.of_string (Buffer.contents buf), List.rev !offset_map, List.rev !label_to_string_and_offset)
...
let emit t =
  ...
  let str_bytes, str_offset_map, label_to_string_and_offset = build_string_table strings in

  (* Emit debug_info using the string offset map *)
  let debug_info_bytes, debug_info_relocs, debug_str_relocs =
    emit_debug_info_with_str_offsets t str_offset_map in

  {
    ...
    debug_str = str_bytes;
    debug_str_labels = label_to_string_and_offset;
